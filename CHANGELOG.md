...
- Judge consult is its own flag. Plugin mode no longer turns the judge off. Live `judge_apply_verdict` is config (`plugins.entries.aegis-atoms.judge_apply_verdict`, default **false**): consult + `aegis-judge.jsonl` for tune; Landen GO flips apply. Sitting quota `AEGIS_JUDGE_SITTING_QUOTA` still caps paid consults.
- Omit-flag test: `evaluate_tool_call` defaults `judge_apply_verdict` to True (engine harness). The live mount defaults False so shadow observations cannot mutate the floor until apply is armed.
