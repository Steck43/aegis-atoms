...
- `SHA256SUMS` is generated by `scripts/gen_manifest.sh` from the git index, and a `manifest` workflow regenerates it on every push, fails on any difference, then runs `sha256sum -c` against a clean checkout. README documents the verify command.
